...
from bloop import BaseModel, Column, DateTime, GlobalSecondaryIndex, Integer, UUID
...
def test_inheritance_double_rangekey(engine):
    with pytest.raises(InvalidModel) as error:
        class NextGenUser(MixinUser):
            version = Column(Integer, range_key=True)
    assert error.value.args == ("'NextGenUser' has more than one range key.",)


def test_inheritance_double_hashkey(engine):
    with pytest.raises(InvalidModel) as error:
        class NextGenUser(MixinUser):
            version = Column(Integer, hash_key=True)
    assert error.value.args == ("'NextGenUser' has more than one hash key.",)


def test_inheritance_two_models_same_dynamo_index_name(engine):
    with pytest.raises(InvalidIndex) as error:
        class NextGenUser(MixinUser):
            version = Column(Integer)
            next_by_email = GlobalSecondaryIndex(projection='all', dynamo_name='email-index', hash_key='email')
    assert error.value.args == ("Index names are not unique: ['email-index'].",)
...
def test_inheritance_two_models_same_dynamo_column_name(engine):
    ...
    assert error.value.args == ("'NextGenUser' has duplicate dynamo_names: ['email'].",)
